...
import com.study.projectboard.domain.type.FormStatus;
...
class ArticleControllerTest {
    ...
    @Test
    void givenNoting_whenRequestingArticlePage_thenToLoginPage()throws Exception{
        Long articleId= 1l;
   ...
   @WithMockUser// 가상의 유저정보가 들어간 척 하게 해주는 어노테이션
    @DisplayName("[view][GET] 게시글 페이지 - 정상 호출, 정상 인증된 사용자")
    @Test
    ...
    @WithUserDetails(value = "minzinoTest",userDetailsServiceBeanName = "userDetailsService",setupBefore = TestExecutionEvent.TEST_EXECUTION)
    ...
    @WithMockUser//로그인된 처리만 필요한 곳에서 사용 로그인 정보를 활용한 테스트에서는 사용x
    @DisplayName("[view][GET] 게시글 수정 페이지- 인증 필요")
    @Test
    ...
    //로그인 정보 를 활용하는 테스트 (userName등 정보를 입력때 필요) 해당 어노테이션을 사용한다.
    @WithUserDetails(value = "minzinoTest",userDetailsServiceBeanName = "userDetailsService",setupBefore = TestExecutionEvent.TEST_EXECUTION)
